...
const Nav = () => {
  ...
  useEffect(() => {
    const changeTitle = () => {
      ...
      if (wS >= hT && wS <= ((hT + aT) / 2)) { setTitleText('Home'); console.log('Home')}
      if (wS >= aT && wS <= ((aT + eT) / 2)) { setTitleText('About Me'); console.log('About') }
      if (wS >= eT) { setTitleText('My Work Experience'); console.log('Experience') }
    }
